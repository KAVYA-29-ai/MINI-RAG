...
def extract_text_from_pdf(file_bytes: bytes) -> str:
    ...
    try:
        ...
        if total_pages > MAX_PAGES:
            logger.warning(f"⚠️ PDF has {total_pages} pages, limiting to {MAX_PAGES}")
        ...
        for i in range(pages_to_process):
            try:
                ...
                if text and text.strip():
                    ...
                    logger.warning(f"⚠️ Page {i+1} has no text (may be scanned)")
                    empty_pages += 1
        ...
        logger.info(f"✅ Extracted {len(full_text)}/{pages_to_process} pages ({empty_pages} failed)")
